...
-- 문자열을 5번째 부터부터 검색하여 두번째 B의 위치 조회
...
SELECT TRIM ('       H E L L O      ') -- 문자들 사이사이에 공백은 제거 안 해
FROM DUAL;
...
SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫쨰자리 아래 절삭
SELECT TRUNC(123.456, -1) FROM DUAL; -- 120, 10의 자리 아래 절삭 3이후로 절삭
...
SELECT SYSDATE FROM DUAL; -- 2025-03-10 12:11:13.000

-- SYSTIMESTAMP : SYSDATE + MS 단취 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:23.248 +0900

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE, '2025-07-22'),3)) "수강 기간( 개월)"
FROM DUAL; -- 4.371 (반올림 + 절대값)
...
-- 0 : 숫자 한칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸을 0으로 채워줌
...
SELECT TO_CHAR(1000000, 'L9,999,999') || '원' FROM DUAL; -- '\' 원화 표시
...
-- EMP_NO (주민번호) : 621231-1985635

-- 1) 주민번호에서 - 앞글자까지 추출
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1) AS 생년월일
FROM EMPLOYEE;

-- 2) 추출한 생년월일을 TO_DATE상태로 변경 -> R 패턴을 이용하여 1900년도로 변경
SELECT EMP_NAME,
...
CASE WHEN SALARY >= 5000000 THEN '대' -- IF문 비슷
...
-- 급여 최대값, 가장 느린 입사일, 알파벳 순서가 가장 느린 이메일 조회
