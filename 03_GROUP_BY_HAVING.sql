...
-- 여러개의 결과값을 산출하기 귀해 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여 사용
...
-- 직급코드가 같은 사람의 (직급을 기준으로 그룹으로 묶기 : JOB_CODE)
-- 직급코드, 급여 평균, 인원 수를	(SELECT 절)
-- 직원코드 오름차순으로 조회 (ORDER BY 절)
...
-- GROUP BY는 별칭 사용X (SELECT 절이 해석되기 전에 수행되기 때문에)
/* 4 */ORDER BY "인원 수"; -- 별칭사용O (SELECT 절이 해석됐기 때문에)
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6') -- 부서코드를 D5, D6로 거름
GROUP BY DEPT_CODE; 
-- WHERE 절에서 구한 부서코드가 D5, D6인 데이터들을 그룹으로 묶음
...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서별로 같은 직급인 사원의 인원 수를 조회 (GROUP BY 절 DEPT_CODE, JOB_CODE 묶음)
-- 부서코드를 오름차순, 직급코드 내림차순으로 정렬 (ORDER BY 절)
-- 부서코드, 직급코드, 인원 수 (SELECT 절)

SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE 
-- DEPT_CODE 로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE 로 그룹을 분류
...
-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 300만원 이상인 부서의 (그룹화 한 부서의 평균 급여)
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순

SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 300만원 이상이라는 조건(요구사항X)
GROUP BY DEPT_CODE
...
-- 집합 연산자 주의사항
-- 집합 연산자를 사용하기 위한 SELECT 문들은
-- 조회하는 컬럼의 타입, 개수가 모두 동일해야 한다.

SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY -- 4개
FROM EMPLOYEE                      --(정수)
WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '가나다' -- 3개, SALARY(정수)와 대응X(문자열) ()
FROM EMPLOYEE                     --(문자열)
WHERE SALARY > 3000000;
...
UNION --(정수)		(문자열)
SELECT DEPT_ID, DEPT_TITLE FROM DEPARTMENT; -- 개수 동일, 타입 같음
