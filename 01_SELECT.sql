...
-- <컬럼 값 산술연산>
-- 컬럼값 : 테이블 내 한칸한칸 (== 한 셀)에 작성된 값(DATA)
...
-- 산술 연산은 숫자타입(NEMBER타입)만 가능하다!

SELECT '같음' FROM DUAL WHERE 1 = '1';
-- ''는 문자열을 의미
-- NUMBER 타입인 1(숫자)과 문자열인 '1'이 같다라고 인식 중.
...
-- CTRL + SHIFT + 위아래방향키 : 줄 이동
-- CTRL + ALT + 위아래방향키 : 줄 복사
...
-- EMPLOYEE 테이블에서
-- 급여가 300만원 이상 500만원 미만의 사원의
-- 사번, 이름, 급여, 전화번호 조회

SELECT EMP_ID, EMP_NAME, SALARY, PHONE
FROM EMPLOYEE
WHERE 3000000 <= SALARY AND SALARY < 5000000;

-- BETWEEN A AND B : A 이상 B 이하

-- EMPLOYEE 테이블에서
-- 급여가 300만원 이상 600만원 미만의 사원의
-- 사번, 이름, 급여, 전화번호 조회
...
-- '_' 예시
-- 'A_' : A로 시작하는 두글자 문자열 EX) AH, AB
-- '____A' : (언더바 4개) : A로 끝나는 5글자 문자열 EX) BCBCA
...
-- EMPLOYEE 테이블에서 이메일에 '_'앞에 글자가 3글자인 사원만
-- 이름, 이메일 조회
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1','D6','D9') -- NULL 미포함 총 12명 조회
OR DEPT_CODE IS NULL; -- 부서코드가 없는 (NULL) 2명 포함 총 14명 조회
 ...
 * DB 에서 NULL : 컬럼에 값이 없음을 의미
 * 
 * 1) IS NILL : NULL인 경우 조회
...
FROM EMPLOYEE
ORDER BY SALARY ; -- ASC(오름차순)가 기본값이라 생략 가능 [ASC | DESC]
-- 컬럼명 사용.

-- EMPLOYEE 테이블에서 (FROM 절)
-- 급여가 200만 이상인 사원의 (WHERE 절)
-- 사번, 이름, 급여 조회 (SELECT 절)
-- 단, 급여 내림차순으로 조회 (ORDER BY 절)
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY >= 2000000
ORDER BY 3 DESC; -- 컬럼 순서 사용. (SALARY를 3번째에 적었으니 ㅎㅎ)
